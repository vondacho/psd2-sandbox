...
> **Status:** evaluation and recommendation produced by AI on 2026-09-19 (iteration 4), answering
...
Two things are bundled in one decision: the **consent screen** (the browser page where the PSU
...
| R3 | Show the PSU what a TPP asks for, and take approve or decline in the browser | `SRC-ADR` Consent screen |
...
| C8 | **PSU trust in the screen** (`UX-03`, phishing resistance) | 3 | **+1** | −1 | Make serves the screen from a bank domain with bank branding. Buy needs white-labelling on a bank domain to match; otherwise the PSU authenticates and consents on a vendor domain |
...
- **Make:** (5×2) + (4×2) + (4×1) + (4×−2) + (3×−2) + (3×1) + (3×1) + (3×1) + (2×2) + (2×−1) + (2×2) = **+27**
- **Buy:** (5×−1) + (4×−1) + (4×0) + (4×2) + (3×2) + (3×0) + (3×−1) + (3×−1) + (2×−2) + (2×1) + (2×−1) = **+1**
...
**Make** — the ASPSP gateway owns the consent state and serves the consent screen, with PSU
authentication delegated to Ping (login screen) and SCA to the CIAM and the bank app.

Three conditions:

1. **Verify the Buy option before committing** (two days, as part of WS-01 preparation). Buy becomes preferable if Finologee can show all of: a webhook or equivalent that reaches the bank *before* the next read after a revocation; white-labelling on a bank domain; an exportable audit trail; and OIDC delegation of PSU authentication to Ping. Record the evidence either way.
...
3. **Keep the screen separable from the state.** If the bank later wants the vendor's screen, the consent state should not move with it. `API-PSU-CHANNEL` is that seam.
...
- `API-PSU-CHANNEL` loses its consent-screen operations and keeps only the SCA-prompt one.
- New questions: revocation propagation latency, audit export, white-label domain, and data processing agreement.
...
- The consent screen is a bank-hosted web application on a bank domain.
